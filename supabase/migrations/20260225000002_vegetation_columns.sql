...
-- ============================================================
-- 4. Seed default vegetation_config for Path C and B+C templates
-- ============================================================
-- Path C (Mapping/WebODM) produces the orthomosaics that Path E processes.
-- Path B+C (Construction + Mapping hybrid) also produces orthomosaics.
-- These templates are pre-configured with conservative defaults so operators
-- can enable vegetation analysis without manual config.
--
-- 'site_survey' and 'environmental_survey' processing paths do not yet exist
-- in processing_templates (they are planned for Phase 12 routing work).
-- This UPDATE targets path_code values that currently produce orthomosaics.
...
WHERE path_code IN ('C', 'B+C');

-- Seed Path E template (new vegetation analysis path)
-- Inserted only if it does not already exist.
...
SELECT
  ...
  'E',
  'Vegetation Analysis',
  'Automated canopy detection, species classification, and health assessment from orthomosaic imagery',
  'vegetation',
  70,
  FALSE,
  FALSE,
  'geojson',
  TRUE,
  '["veg_canopy_detection", "veg_species_classification", "veg_health_assessment", "veg_report_generation"]'::jsonb,
  TRUE,
  '{
    "tile_size": 1024,
    "score_threshold": 0.3,
